...
int main()
{
    ...
    for (const auto& item : topLevelNodes) {
        //TODO: FunctionStatementのrangeが正しくない。
        //というかあまりRangeを厳密に考えてないので、この際ツールのためにちゃんとする
        if (item->GetType() == sakura::ASTNodeType::FunctionStatement) {
            auto f = sakura::JsonSerializer::MakeObject();
            //TODO: 関数ブロックレンジをまとめて取り出す必要がある
            //      そもそも今のソースコードレンジの数え方に自身がないので、みてみるほかない
            //f->Add("line", sakura::JsonSerializer::From(item->GetSourceRange().GetBeginLineIndex()));
