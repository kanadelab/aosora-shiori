...
#if 0
			.Register("ERROR_MESSAGE_A001", "コードブロック終了の閉じ括弧 } が見つかりませんでした。")
			.Register("ERROR_HINT_A001", "コードブロックは { } で囲まれる一連の処理ですが、始まりに対して終わりが見つからないエラーです。コードブロックや、付近の { の閉じ括弧を忘れていないか確認してください。")
			.Register("ERROR_MESSAGE_A002", "演算子の使い方が正しくありません。")
			.Register("ERROR_HINT_A002", "2つの要素を足したり掛けたりするタイプの計算式で、足す側と足される側といったような２つの要素が揃っていないようです。")
			.Register("ERROR_MESSAGE_A003", "カッコの対応関係が間違っています。")
			.Register("ERROR_HINT_A003", "計算式に使うカッコ ( ) の対応関係が正しくないようです。計算式を確認してください。")
			.Register("ERROR_MESSAGE_A004", "演算子が必要か、ここでは使えない演算子です。")
			.Register("ERROR_HINT_A004", "値を2つ連続することはできません。足し合わせるなら + を使うなど、式にする必要があります。")
			.Register("ERROR_MESSAGE_A005", "メンバ名を指定する必要があります。")
			.Register("ERROR_HINT_A005", "Value.Item のように、ピリオドはオブジェクトのメンバーを参照するために使用します。")
			.Register("ERROR_MESSAGE_A006", "ここでセミコロン ; は使えません。")
			.Register("ERROR_HINT_A006", "この式ではセミコロン ; を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A007", "ここでコロン : は使えません。")
			.Register("ERROR_HINT_A007", "この式ではコロン : を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A008", "ここで閉じ括弧 } は使えません。")
			.Register("ERROR_HINT_A008", "この式では閉じ括弧 } を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A009", "ここで閉じ括弧 ] は使えません。")
			.Register("ERROR_HINT_A009", "この式では閉じ括弧 ] を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A010", "ここでカンマ , は使えません。")
			.Register("ERROR_HINT_A010", "この式ではカンマ , を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A011", "ここで閉じ括弧 ) は使えません。")
			.Register("ERROR_HINT_A011", "ここでは閉じ括弧 ) を使うことができません。機能の使い方が間違ってないか、確認してみてください。")
			.Register("ERROR_MESSAGE_A012", "カッコの対応関係が正しくありません。")
			.Register("ERROR_HINT_A012", "リストの始端終端のカッコが揃っていないようです。")
			.Register("ERROR_MESSAGE_A013", "引数名が必要です。")
			.Register("ERROR_HINT_A013", "ここは引数リストなので、引数名を書かないといけません。")
			.Register("ERROR_MESSAGE_A014", "引数リストが正しくありません。")
			.Register("ERROR_HINT_A014", "カンマ , で引数を区切るか、閉じ括弧 ) で引数リストを閉じる必要があります。")
			.Register("ERROR_MESSAGE_A015", "カッコの対応関係が正しくありません。")
			.Register("ERROR_HINT_A015", "引数リストが正しく閉じられていません。")
			.Register("ERROR_MESSAGE_A016", "キーが必要です。")
			.Register("ERROR_HINT_A016", "連想配列の式では キー: 値　の形式で内容を記述します。キーが正しい形式ではないようです。")
			.Register("ERROR_MESSAGE_A017", "コロン : が必要です。")
			.Register("ERROR_HINT_A017", "連想配列の式では キー: 値 の形式で内容を記述します。キーと値を分けるコロンが無いようです。")
			.Register("ERROR_MESSAGE_A018", "開き括弧 { が必要です。")
			.Register("ERROR_HINT_A018", "関数式では { から関数本体を始める必要があります。")
			.Register("ERROR_MESSAGE_A019", "開き括弧 { が必要です。")
			.Register("ERROR_HINT_A019", "トーク式では { からトーク本体を始める必要があります。")
			.Register("ERROR_MESSAGE_A020", "関数名が必要です。")
			.Register("ERROR_HINT_A020", "関数定義ではfunction に続けて関数名が必要です。")
			.Register("ERROR_MESSAGE_A021", "開き括弧 ( が必要です。")
			.Register("ERROR_HINT_A021", "関数定義の発生条件を使用する場合は if に続けて括弧 ( ) で条件を記述します。")
			.Register("ERROR_MESSAGE_A022", "開き括弧 { が必要です。")
			.Register("ERROR_HINT_A022", "関数定義では { から関数本体を始める必要があります。")
			.Register("ERROR_MESSAGE_A023", "書き込めない対象に書き込もうとしています。")
			.Register("ERROR_HINT_A023", "読み取り専用の情報に代入等で変更をしようとしています。")
			.Register("ERROR_MESSAGE_A024", "newキーワードには呼出式 ( ) が必要です。")
			.Register("ERROR_HINT_A024", "new Class() のように、newキーワードには呼出式が必要です。")
			.Register("ERROR_MESSAGE_A025", "クラス名が必要です。")
			.Register("ERROR_HINT_A025", "クラス定義では class キーワードに続いてクラス名を記述します。")
			.Register("ERROR_MESSAGE_A026", "継承クラス名が必要です。")
			.Register("ERROR_HINT_A026", "クラスを継承する場合はクラス名の後のコロン : に続けて継承元のクラス名を記述します。")
			.Register("ERROR_MESSAGE_A027", "開き括弧 { が必要です。")
			.Register("ERROR_HINT_A027", "クラス本体の記述前に始端の括弧 { が必要です。")
			.Register("ERROR_MESSAGE_A028", "開き括弧 { が必要です。")
			.Register("ERROR_HINT_A028", "initの本体の前に始端の括弧 { が必要です。"),
			.Register
#endif
	}
